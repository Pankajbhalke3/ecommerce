...
app.post("/upload",upload.single('product'),(req,res)=>{
    res.json({
        success:1,
        image_url:`https://shopper-xmp2.onrender.com/images/${req.file.filename}`
    })
})
...
const fetchUser = async (req,res,next)=>{
    ...
    if(!token){
        res.status(401).send({errors:"Please authenticate using valida token"})
    }
    else
    {
        try{
...
app.post('/placeorder', fetchUser, async (req, res) => {
    ...
    if (!user || !cartItems || totalAmount === undefined) {
    ...
    try {
        ...
        // ✅ Clear cart in database after order placed
        const dbUser = await Users.findById(req.user.id);
        if (dbUser) {
            dbUser.cartData = {}; // Clears backend cart
            await dbUser.save();
        }
        ...
        console.error(err);
        res.status(500).json({ success: false, message: "Order failed due to server error." });
    }
});
